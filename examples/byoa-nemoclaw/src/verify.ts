// Verify the MCP path the sandboxed agent will use — without involving
// NemoClaw or whatever agent it's running.
//
// The sandboxed agent spawns `pnpm exec agentbase-mcp` over stdio (see
// nemoclaw-mcp-config.yaml). This script spawns that exact command with your
...
//
// This replays the same "delusional agent" beat as byoa-openclaw/byoa-hermes:
// NemoClaw's network policy tier is what decides whether the sandbox can
// reach HubSpot at all — assume it correctly says yes. That tells you nothing
// about whether this specific deal-update call should execute. This script
// proves Agentbase still gates it even after the sandbox's own perimeter has
// already let the call through.
//
// If this prints "awaiting_approval" for hubspot_deals_update, the gate is
// live and it's safe to point the sandboxed agent at it. Run:
//
...
// Spawn the *exact* command the sandbox's MCP config uses, so this verifies
// the real wiring rather than a bespoke one.
...
const client = new Client({ name: 'byoa-nemoclaw-verify', version: '0.0.0' });
...
// hubspot.deals.update over $25k requires approval under policy.yaml — the
// sandbox's network tier already let this call reach the gate; Agentbase is
// what still stops it from executing unreviewed.
...
const result = await client.callTool({
  ...
  arguments: {
    ...
    idempotency_key: `byoa-nemoclaw-${Date.now()}`,
  },
});
