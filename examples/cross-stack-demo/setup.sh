#!/usr/bin/env bash
# Wires the cross-stack demo: applies the cross-stack policy.yaml, registers
...
echo "→ Installing cross-stack policy …"
curl -fsS -X PUT "$API/v1/policies/active" \
  -H 'content-type: application/json' \
  -d "$(jq -n --arg yaml "$POLICY_YAML" '{name: "cross-stack-demo", yaml: $yaml}')" \
...
REG=$(curl -fsS -X POST "$API/v1/agents" \
  -H 'content-type: application/json' \
  -d '{"name":"cross-stack-demo","description":"reference agent governed across HubSpot + Salesforce + Gmail under one policy"}')
...
echo "  pnpm --filter '@agentbase/cross-stack-demo' exec tsx src/index.ts"
echo
